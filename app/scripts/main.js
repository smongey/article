...
$(document).ready(function(){
	...
	// Determines if the page is a slider page or not
	if ($('body').hasClass('products') || $('body').hasClass('details')){

		// nada because neither products or details has a slider
		...
		if ( $('body').hasClass('home') || $('body').hasClass('story') ) {
			...
			$('#slidecount').css({ top : $('#slider').height() - 100 });
		
		// this is the sinlge product slider code
		...
		// General slider required code

		$('a#prev').on('click', function(e){ e.preventDefault(); mySwipe.prev(); });
		$('a#next').on('click', function(e){ e.preventDefault(); mySwipe.next(); });
	...
	logoBox.hover(function(){
		circleLogo.animate({ r : 40, opacity: '.7' }, 250, 'elastic');
	}, function(){
		circleLogo.animate({ r : 30, opacity: 1 }, 350, 'elastic');
	});

	cartBox.hover(function(){
		cartCircle.animate({ r : 35, opacity: 1 }, 250, 'elastic');
	}, function(){
		cartCircle.animate({ r : 30, opacity: '.7' }, 350, 'elastic');
	});
	...
	// JUST HOME AND PRODUCTS
	} else if ($('body').hasClass('home') || $('body').hasClass('products')) {

		$('a.eye').hover(function(){
